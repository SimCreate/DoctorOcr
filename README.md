...
Kaggle RxHandBD 데이터셋(5,578장) 기반, v1 → v3_3 순차 실험 (모노레포)

- 현재 메인: **doctor_ocr_v3_3** (resnet18 + BiLSTM3 + 하이브리드 Attention/CTC, 라벨 정제 적용, 자립 패키지)
...
├── .gitignore
├── doctor_ocr_v3_3/              # v3_3 — ★ 현재 메인 (하이브리드 resnet18+Attention+CTC, 자립)
...
├── doctor_ocr_v3_2/              # v3_2 — 보관 (resnet18 백본 + Attention) — 상세는 [Legacy 기록]
├── legacy/                       # ★ 보관 — 상세는 하단 [Legacy 기록]
│   ├── doctor_ocr_v2_2/
│   └── doctor_ocr_v3/
└── (v1/v2/v2.1/v3_1: 개별 트리, [Legacy 기록] 참조)
```
...
**현재 결과 (v3_3e = 메인, 하이브리드 + 라벨 정제, 클린 val 1,116장)**

| 디코더 | exact | CER | 판정 |
|---|---|---|---|
| **CTC greedy** | **48.2%** | **24.3%** | FAIL |
| Attention beam(200) (로그) | 67.0% | — | (정식 eval 미실행) |
| Attention beam5 (v3_3d) | 56.5% | 24.5% | — |

- 라벨 정제(소문자 통일·하이픈→공백)로 CTC greedy 45.5% → **48.2%** (+2.7p). CER 25.8% → **24.3%**.
- 빈도그룹(e): 고 70.3% (n=684) / 중 23.6% (n=216) / 저 2.8% (n=216). **저빈도 붕괴가 유일한 한계** — 디코더가 아니라 데이터/라벨 문제로 확정.
- ⚠️ 라벨 정제로 빈도그룹 경계가 이동해, 이전 버전과 그룹별 수치는 직접 비교 불가 (전체 exact만 동일 val 기준 비교).
- **attention 정식 eval은 아직 미실행** (eval 스크립트 `--attn` 경로는 stub). 67.0%는 학습 로그의 beam(200) 값.
- 상세: `doctor_ocr_v3_3/DESIGN.md`

**버전 계보 (활성: v1 → v2 → v2.1 → v3_1 → v3_2 → v3_3)**

| 항목 | v3_1 | v3_2 | v3 CTC | v3_3(버그) | v3_3d(수정) | **v3_3e(+라벨정제)** |
|---|---|---|---|---|---|---|
...
**현재 메인: doctor_ocr_v3_3 — 하이브리드 (resnet18 + Attention + CTC)**
...
- **Attention**: 언어 prior → 고빈도/중빈도 패턴 인식 (beam5 56.5%로 CTC 45.5%보다 우세)
...
의료 처방전 OCR은 단어 단위 평가가 실제 활용에 맞다 — 이미지가 아니라 **출력된 단어 전체가 정확한가**를 본다.
...
- 원본 보관: `doctor_ocr_v3_3/backups/val_ORIGINAL_20260807_170138.csv`, `combined_labels_ORIGINAL_20260807_170138.csv`
- 효과: exact +2.7p (45.5→48.2), CER -1.5p (25.8→24.3)

**클린 split**: v3_1부터 원본 80/20을 먼저 고정 분리 후 train에만 증강 — val 1,116장이 어떤 train에도 포함되지 않게 보장 (리키지 차단). v3_3은 이 클린 val 1,116 고정.

## 실행 방법

> 스크립트 내 경로는 라이브 디렉토리(`/home/dev/DoctorOcr/`) 기준. GPU1(Max-Q)은 vLLM과 공존.
...
- **v3_3 학습 산출물**: best `working/checkpoints/best_model_v3_3.pth` (epoch/optimizer/config 포함), 최신 실험 `best_model_v3_3e.pth` 보관
- 문자 사전: `working/char_dict_v3_3.pkl` (char2idx, idx2char)

## 알려진 이슈 / 교훈

- **저빈도 붕괴** (v3_3e 저 2.8%): 모든 디코더 공통 — 디코더/beam이 아니라 **데이터/라벨/외부 손글씨** 문제. 다음 단계 핵심 타깃
- **attention beam5 정식 eval 미실행**: v3_3e의 attention 67%(beam200)는 로그값. eval 스크립트 `--attn`은 stub
- **빈도그룹 경계 이동**: 라벨 정제로 train 빈도가 바뀌어 이전 버전과 그룹별 비교 불가 (전체 exact만 비교)
- **시퀀스 시프트 버그**: attention의 입력/정답 시프트 일치가 생명 (위 상세)
- **학습 로그 acc ≠ 실제 디코딩**: 최종 판단은 반드시 디코더 재평가 (v3_1에서 검증된 교훈)

## Legacy 기록

### v3_2 — resnet18 백본 + Attention (보관)
`doctor_ocr_v3_2/` — v3_1의 SEBlock 인코더를 ImageNet pretrained resnet18로 교체 + 256x128 비율유지 패딩.
결과: exact 34.7% / CER 39.4% (v3_1보다 개선, but v3_3 하이브리드에 흡수)

### v1 / v2 / v2.1 / v3_1 — 개별 트리 (보관)
- `doctor_ocr/` (v1) — 기준 CRNN (Attention Decoder) — 실학습 89장 한계
- `doctor_ocr_v2/` (v2) — 개선 CRNN (대용량 배치 + AMP)
- `doctor_ocr_v2_1/` (v2.1) — BiLSTM 3층 + Attention 8-head + Beam Search
- `doctor_ocr_v3_1/` (v3_1) — SEBlock + 증강 데이터 + attention — exact 30.4%
