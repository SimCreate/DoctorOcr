...
| **v4 — 하이브리드 (현재)** ★ | `v4/` | resnet18 + Attention + **CTC** 이중헤드, 라벨 정제 | **48.2%** (attn beam200 67.0%) |

- v1→v2: 데이터셋을 89장 → 5,578장으로 확장하고 아키텍처를 개선
- v2→v3: 데이터 가공(클린 split·증강)으로 학습량 13,386장까지 확대, 백본을 resnet18로 전환
- v3→v4: CTC+Attention 하이브리드 구조로 전환 + 라벨 정제 → 프로젝트 사상 최고
...
**현재 결과 (v4 = 메인, 하이브리드 + 라벨 정제, 클린 val 1,116장)**
...
- 빈도그룹: 고 70.3% (n=684) / 중 23.6% (n=216) / 저 2.8% (n=216). **저빈도 붕괴가 유일한 한계** — 디코더가 아니라 데이터/라벨 문제로 확정.
...
- **저빈도 붕괴** (v4 저 2.8%): 모든 디코더 공통 — 디코더/beam이 아니라 **데이터/라벨/외부 손글씨** 문제. 다음 단계 핵심 타깃
- **attention beam5 정식 eval 미실행**: v4의 attention 67%(beam200)는 로그값. eval 스크립트 `--attn`은 stub
