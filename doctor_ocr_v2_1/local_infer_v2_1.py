#!/usr/bin/env python3
"""
Doctor Handwriting OCR - Local Inference Script (v2_1)
개선된 추론: Beam Search, 후처리, 배치 처리 지원
"""
...
def parse_args():
    ...
    parser.add_argument('--beam-width', type=int, default=5, help='Beam search width (default: 5)')
...
# ============================================================
# CONFIG - /home/dev/doctor_ocr_v2_1/ 경로 (RTX PRO 6000 Blackwell — cuda0)
# ============================================================
...
def postprocess_prediction(pred_str):
    """후처리: 중복 문자 제거, 공백 정리"""
    # 연속된 중복 문자 제거
    result = []
    prev_char = None
...
def load_model_and_dict():
    ...
    model = CRNN(
        vocab_size,
        hidden_size=384,  # model_v2_1 정의와 일치 (체크포인트 config는 256으로 잘못 저장됨)
...
def predict_single(model, img_tensor, idx2char, beam_width=5):
    """Beam search 기반 예측"""
    with torch.no_grad():
        img_tensor = img_tensor.to(DEVICE)
        if beam_width > 1:
            pred = model.predict(img_tensor, beam_width=beam_width)
            pred_str = decode_sequence(pred[0].cpu().tolist(), idx2char)
        else:
            output = model(img_tensor, teacher_forcing_ratio=0.0)
            pred = output.argmax(-1)[0].cpu().tolist()
            pred_str = decode_sequence(pred, idx2char)

        # 후처리
        pred_str = postprocess_prediction(pred_str)
        return pred_str
...
def main():
    ...
    print(f"\n=== INFERENCE ON {len(samples)} RANDOM SAMPLES (Beam Search) ===\n")
    ...
    for row in samples:
        ...
        pred_label = predict_single(model, img_tensor, idx2char, beam_width=args.beam_width)
