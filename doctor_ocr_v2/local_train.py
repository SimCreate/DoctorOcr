#!/usr/bin/env python3
"""
Doctor Handwriting OCR - Local Training Script (v2)
Paths configured for /home/dev/doctor_ocr_v2/
"""
...
# ============================================================
# CONFIG - HARDCODED PATHS FOR /home/dev/doctor_ocr_v2/
# ============================================================
DATA_ROOT = Path("/home/dev/doctor_ocr_v2/dataset")
IMG_DIR = DATA_ROOT / "img" / "img"
LABEL_CSV = DATA_ROOT / "doctor_handwriting_labels.csv"

WORK_DIR = Path("/home/dev/doctor_ocr_v2/working")
CHECKPOINT_DIR = WORK_DIR / "checkpoints"
CHAR_DICT_PATH = WORK_DIR / "char_dict.pkl"
BEST_MODEL_PATH = CHECKPOINT_DIR / "best_model.pth"

# 학습 하이퍼파라미터 (RTX PRO 6000 Blackwell Workstation — cuda0, 192GB VRAM, USB4 4060Ti 제거)
BATCH_SIZE = 96              # Blackwell workstation: 192GB VRAM, 배치 96 유지 (AMP 필수)
ACCUM_STEPS = 1              # 배치 키워서 accum 불필요 (유효 배치 96)
NUM_EPOCHS = 50
LR = 3e-4                    # 배치 커졌으니 LR 상향
WEIGHT_DECAY = 1e-4
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
# Blackwell RTX PRO 6000 enforced via CUDA_VISIBLE_DEVICES='0'; cuda:0 → physical GPU 0
NUM_WORKERS = 8              # CPU 전처리 병렬화 최대화
PIN_MEMORY = True

# AMP (Automatic Mixed Precision) - 필수 (속도 + VRAM 절약)
USE_AMP = True
...
IMAGE_WIDTH = 256
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
print(f"[CONFIG] DEVICE: {DEVICE} | CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES','unset')} | Mapped GPU: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'N/A'}")
if DEVICE.type == "cuda":
    print(f"[CONFIG] GPU: {torch.cuda.get_device_name(0)}, Free VRAM: {torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_allocated(0) >> 20} MiB")
...
def main():
    ...
    # Single GPU mode (DataParallel disabled for speed on this model size)
    print(f"[MODEL] Single GPU mode")
    criterion = nn.CrossEntropyLoss(ignore_index=SPECIAL_TOKENS['<PAD>'])
    optimizer = optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)

    # Resume: load model and optimizer state
    if resume_from and Path(resume_from).exists():
        ckpt = torch.load(resume_from, map_location=DEVICE)
    ...
    # Adjust scheduler to resume epoch
    for _ in range(start_epoch - 1):
        scheduler.step()

    print(f"[RESUME] Starting from epoch {start_epoch}, LR={scheduler.get_last_lr()[0]:.2e}")
