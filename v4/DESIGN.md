...
> 상태: **Attention beam5 62.3% / CTC greedy 48.2% — 프로젝트 사상 최고 (정식 eval 확정)**
...
| 전체 exact (Attn beam5) | 30.4% | 34.7% | — | — | **56.5%** | **62.3%** |
...
- **v3_3e (라벨 정제) = attention beam5 62.3% (+5.8p) / CTC greedy 48.2% (+2.7p)**: 라벨 정제가 양 디코더 모두 개선
- **v3_3e 정식 eval** (val 1,116장, beam5): attention **62.3%** (CER 22.9%, 고 83.9%/중 55.1%/저 0.9%), CTC **48.2%** (CER 24.3%)
- **attention이 CTC보다 전반 우세** — 고·중빈도에서 특히 강함 (중빈도 55.1 vs 23.6)
- **저빈도 붕괴** (attn 0.9% / ctc 2.8%) 는 모든 디코더 공통 — 디코더가 아니라 **데이터/라벨 문제** 확정
...
- OCR/HWR에서 **Hybrid(CTC+Attention 공동손실)**는 "효과 유망하나 검증 필요" — 본 프로젝트로 검증 완료: **유효** (attention beam5 62.3%)
...
- **v3_3e (라벨 정제, 정식 eval)**: attention beam5 == **62.3%** (CER 22.9%), CTC greedy == **48.2%** (CER 24.3%)
- 빈도그룹(e, attention): 고 83.9% / 중 55.1% / 저 0.9% — attention이 CTC(고70.3/중23.6/저2.8)보다 고·중빈도 우세
- ⚠️ 빈도그룹(n): e는 고 684 / 중 216 / 저 216 — **그룹 경계가 d와 달라 그룹별 직접 비교 불가**
...
### 달성
- attention beam5 **62.3%** (정식 eval), CTC greedy **48.2%** — 70% 목표에 근접
- **라벨 정제가 주목할 개선** — attention(56.5→62.3)과 CTC(45.5→48.2) 모두 개선

### 남은 리스크
1. **저빈도 붕괴** (attn 0.9% / ctc 2.8%) — 디코더 무관, **데이터 확충/외부 손글씨 사전학습** 필요 (다음 단계 핵심)
2. **attention의 고빈도 편중** — 고빈도 83.9% vs 중빈도 55.1% vs 저빈도 0.9%. 중·저빈도 격차가 여전히 큼
3. **빈도그룹 경계가 라벨 정제로 이동** — 이전 버전과 그룹별 비교가 더 이상 1:1 안 됨 (문서·보고서에 명시할 것)
4. attention beam eval 비용: val 전체 1,116장에 beam5 = 수 분 (최종 eval만, 이미 수행)
...
| 단계 | 핵심 변경 | 결과 |
|---|---|---|
| v3_3 | 하이브리드 첫 도입 | CTC 45.4% |
| v3_3b | TF 일정 수정 | (중간) |
| v3_3c | TF 0.2→0.05, CE scale β=3.0 | (중간) |
| v3_3d | 시퀀스 시프트 버그 수정 | CTC 45.5%, attn beam5 56.5% |
| **v3_3e** | **라벨 정제 (메인)** | **CTC 48.2%, attn beam5 62.3% (정식 eval)** |
...
**eval 결과**: `evaluate/result_v3_3e_clean.csv` (CTC greedy 48.2%) · `evaluate/result_v3_3e_clean_attn.csv` (attention beam5 62.3%)

---

## 8. 다음 단계 후보 (저빈도 0.9~2.8% 돌파가 메인)

- ✅ **라벨 정제** — v3_3e에서 수행 완료. attention·CTC 모두 개선. 원본은 `backups/`에 보관
- ✅ **attention beam5 정식 eval** — 62.3% 확정

1. **두 디코더 앙상블/선택** — attention 62.3% & CTC 48.2% 조합으로 상호 보완 (저빈도 방향성 확인)
