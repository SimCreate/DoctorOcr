...
sys.path.insert(0, str(V3_2 / "scripts"))

from model_v3_2 import CRNN, decode_sequence, MAX_LABEL_LENGTH
from preprocess_v3_2 import load_resize_pad, preprocess_tensor

# ---- v3_2 자체 지표 모듈 (기본 지표는 v3에서 복제, v3_1 전용 확장 지표 포함) ----
sys.path.insert(0, str(V3_2 / "evaluate"))
...
def parse_args():
    ...
    p.add_argument('--ckpt', type=str, default=str(V3_2 / "working/checkpoints/best_model_v3_2.pth"))
    p.add_argument('--val-csv', type=str, default=str(V3_2 / "data/clean_split/val.csv"))
    p.add_argument('--img-dir', type=str, default="/home/dev/doctor_ocr_v2/dataset/img/img")
    p.add_argument('--char-dict', type=str, default=str(V3_2 / "working/char_dict_v3_2.pkl"))
    p.add_argument('--out', type=str, default=str(V3_2 / "evaluate/result_v3_2_clean.csv"))
...
def main():
    ...
    def load_img(path: Path):
        # v3_2 전처리: 비율유지 패딩 + ImageNet normalized tensor (훈련과 동일)
        img = load_resize_pad(path, model_h, model_w)
        return preprocess_tensor(img)   # (3,H,W) float

    model_h = config.get('image_height', 128)
    model_w = config.get('image_width', 256)
    print(f"[DATA] val rows: {len(val_df)}  (img {model_w}x{model_h}, 비율유지 패딩)")
    ...
    with torch.no_grad():
        for i, (_, row) in enumerate(val_df.iterrows()):
            ...
            img_t = torch.from_numpy(img_t).float().unsqueeze(0).to(device)   # [1,3,H,W]
    ...
    train_csv = str(V3_2 / "data/exp2_clean/combined_labels.csv")
