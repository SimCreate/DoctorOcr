...
실행 (GPU1 Max-Q, vLLM 공존):
  CUDA_VISIBLE_DEVICES=1 venv/bin/python scripts/train_v3_3d.py
...
BEST_MODEL_PATH = CHECKPOINT_DIR / "best_model_v3_3d.pth"
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            periodic_path = CHECKPOINT_DIR / f"epoch_v3_3d_{epoch}.pth"
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    print(f"\n[DONE] v3_3d complete. best_hybrid={best_hybrid_score:.4f}, best_attn={best_attn_acc:.4f}")
