...
실행 (GPU1 Max-Q, vLLM 공존):
  CUDA_VISIBLE_DEVICES=1 venv/bin/python scripts/train_v3_3c.py
...
BEST_MODEL_PATH = CHECKPOINT_DIR / "best_model_v3_3c.pth"
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            periodic_path = CHECKPOINT_DIR / f"epoch_v3_3c_{epoch}.pth"
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
