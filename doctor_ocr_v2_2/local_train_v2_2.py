#!/usr/bin/env python3
"""
Doctor Handwriting OCR - Local Training Script (v2_2 CTC)
CTC Loss 기반 학습: 병렬 연산으로 GPU 100% 활용, Warmup + Cosine Annealing,
Mixed Precision, Gradient Accumulation, 데이터 증강, Early Stopping
"""
...
# 학습 하이퍼파라미터 (RTX PRO 6000 Blackwell — cuda0, USB4 제거)
BATCH_SIZE = 8               # CTC는 메모리 효율적이라 배치 더 키움 가능
ACCUM_STEPS = 16             # Effective batch size = 128
NUM_EPOCHS = 80
LR = 3e-4
WEIGHT_DECAY = 1e-4
WARMUP_EPOCHS = 5
...
# Blackwell enforced via CUDA_VISIBLE_DEVICES='0' (physical GPU 0 mapped to cuda:0)
NUM_WORKERS = 4
PIN_MEMORY = True
...
# ============================================================
# DATASET (CTC용: variable length targets 처리)
# ============================================================
class HandwritingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CTC 타겟: 변수 길이 시퀀스 (blank token 제외)
...
def ctc_collate_fn(batch):
    """CTC용 collate: 이미지는 stack, 타겟은 concat + lengths 반환"""
    ...
    for step, (imgs, targets, target_lengths) in enumerate(pbar):
        ...
        if use_amp and device.type == "cuda":
            with torch.amp.autocast('cuda'):
                logits = model(imgs)  # [B, T, vocab_size]
...
def main():
    ...
    print(f"[VOCAB] chars: {list(char2idx.keys())}")
    ...
    model = CRNN(vocab_size, hidden_size=256, dropout=DROPOUT, use_gradient_checkpointing=USE_GRADIENT_CHECKPOINTING).to(DEVICE)
